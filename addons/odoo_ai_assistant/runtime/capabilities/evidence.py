...
rechecks access and freshness whenever a reference is resolved.
"""
...
def redact_secrets(value: str) -> str:
    """Redact common credential shapes without blocking normal conversation."""
    ...
    *,
    ...
    if isinstance(value, Mapping):
        ...
        for key in sorted(value, key=lambda item: str(item)):
            ...
            counter["keys"] += 1
            if counter["keys"] > MAX_JSON_KEYS_TOTAL:
                raise CapabilityError("evidence_json_keys_exceeded")
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        for item in value:
            counter["items"] += 1
            if counter["items"] > MAX_JSON_ITEMS_TOTAL:
...
def _context_company_ids(context: CapabilityContext) -> tuple[int, ...]:
    ...
    ids = getattr(getattr(env, "companies", None), "ids", ())
...
class EvidenceAccessScope:
    """Identity binding checked both at collection and fetch time."""
...
class EvidenceLocator:
    """Host-created logical locator; never a command or arbitrary absolute path."""
...
class EvidenceItem:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "excerpt",
            _bounded_text(self.excerpt, limit=DEFAULT_MAX_EXCERPT_BYTES, code="evidence_excerpt_too_large"),
        )
        object.__setattr__(self, "data", freeze_json_mapping(self.data, max_bytes=DEFAULT_MAX_EXCERPT_BYTES))
...
class EvidenceRoutingPolicy:
    """Prioritize evidence classes without classifying the complete user intent."""
...
class EvidenceLedgerSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        for item in self.items:
            canonical_ref = refs_by_id.get(item.ref.evidence_id)
            if canonical_ref is None:
                raise CapabilityError("evidence_ledger_orphan_item")
            if (
                canonical_ref.provider_id != item.ref.provider_id
                or canonical_ref.locator.canonical_key != item.ref.locator.canonical_key
            ):
        ...
        *,
        ...
        if isinstance(value, EvidenceItem) and retain_excerpt:
            ...
            if len(bounded_excerpt.encode("utf-8")) > LEDGER_MAX_EXCERPT_BYTES:
                bounded_excerpt = bounded_excerpt.encode("utf-8")[:LEDGER_MAX_EXCERPT_BYTES].decode(
                    "utf-8",
                    errors="ignore",
                )
            ...
            proposed_items[ref.evidence_id] = item
        captured_at = utcnow()
        ordered_items = tuple(
