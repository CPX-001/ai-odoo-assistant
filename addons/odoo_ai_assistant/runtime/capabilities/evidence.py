...
Evidence is contextual data, never executable authority. Providers expose logical
locators and bounded projections; the host rechecks access and freshness whenever
...
MAX_JSON_DEPTH = 12
MAX_JSON_KEYS_PER_OBJECT = 256
MAX_JSON_KEYS_TOTAL = 8192
MAX_JSON_ITEMS_TOTAL = 4096
...
def freeze_json(value: Any, *, max_bytes: int = MAX_JSON_BYTES) -> JsonValue:
    ...
    frozen = _freeze_json(value, depth=0, counters=counters, secret_key=False)
...
def _freeze_json(
    ...
    *,
    ...
    if isinstance(value, str):
        ...
        if len(text.encode("utf-8")) > DEFAULT_MAX_TOTAL_BYTES:
            raise CapabilityError("evidence_string_too_large")
        return text
    if isinstance(value, Mapping):
        if len(value) > MAX_JSON_KEYS_PER_OBJECT:
        ...
        for key in sorted(value, key=lambda item: str(item)):
            ...
            counters["keys"] += 1
            if counters["keys"] > MAX_JSON_KEYS_TOTAL:
                raise CapabilityError("evidence_json_keys_exceeded")
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        for item in value:
            counters["items"] += 1
            if counters["items"] > MAX_JSON_ITEMS_TOTAL:
...
def _context_company_ids(context: CapabilityContext) -> tuple[int, ...]:
    ...
    ids = getattr(
        getattr(getattr(context, "env", None), "companies", None),
        "ids",
        (),
    )
...
def _context_group_xmlids(context: CapabilityContext) -> tuple[str, ...]:
    ...
    return tuple(sorted({str(item) for item in explicit or () if item}))
...
class EvidenceAccessScope:
    ...
    def from_json_value(cls, value: Mapping[str, Any]) -> "EvidenceAccessScope":
        return cls(
            ...
            group_xmlids=tuple(value.get("group_xmlids") or ()),
            source_acl=tuple(value.get("source_acl") or ()),
        ...
        *,
        ...
        if not self.public:
            if self.user_id is not None and self.user_id != _context_user_id(context):
                return False
            if self.company_ids and not set(self.company_ids).issubset(
                set(_context_company_ids(context))
            ):
                return False
...
class EvidenceLocator:
    ...
    def canonical_key(self) -> str:
        return (
            f"{self.provider_id}:{self.source_id}:{self.key}:"
            f"{canonical_fingerprint(self.parameters)[:16]}"
        )
...
class EvidenceRef:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "title", _bounded_text(self.title, limit=320, code="evidence_title_invalid"))
        object.__setattr__(
            self,
            "provenance",
            _bounded_text(self.provenance, limit=1024, code="evidence_provenance_invalid"),
        )
        fingerprint = _bounded_text(self.fingerprint, limit=256, code="evidence_fingerprint_invalid")
        if not fingerprint:
        ...
        object.__setattr__(
            self,
            "conflict_group",
            _bounded_text(self.conflict_group, limit=160, code="evidence_conflict_group_invalid"),
        )
        object.__setattr__(self, "citation", freeze_json_mapping(self.citation, max_bytes=8 * 1024))
    ...
    def from_json_value(cls, value: Mapping[str, Any]) -> "EvidenceRef":
        return cls(
            ...
            provider_id=str(value["provider_id"]),
            locator=EvidenceLocator.from_json_value(value["locator"]),
            ...
            trust=EvidenceTrust(value["trust"]),
            access_scope=EvidenceAccessScope.from_json_value(value["access_scope"]),
            citation=value.get("citation") or {},
    ...
    def to_json_value(self, *, include_scope: bool = True) -> dict[str, JsonValue]:
        ...
        if include_scope:
            result["access_scope"] = self.access_scope.to_json_value()
        return result
...
class EvidenceItem:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "data",
            freeze_json_mapping(self.data, max_bytes=DEFAULT_MAX_TOTAL_BYTES),
        )
        if len(canonical_json_bytes(self.to_untrusted_projection())) > DEFAULT_MAX_TOTAL_BYTES:
...
class EvidenceSearchRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "metadata", freeze_json_mapping(self.metadata, max_bytes=8 * 1024))
...
class EvidenceRoutingPolicy:
    ...
    def select(
        ...
        def key(provider: EvidenceProvider) -> tuple[int, str]:
        ...
        return tuple(sorted(ordered, key=key))
...
class EvidenceProviderCatalog:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, EvidenceProviderCatalog):
        ...
        for provider in self._providers:
            if not provider.default_enabled:
                if not provider.optional:
                    raise CapabilityError("required_evidence_provider_disabled")
                statuses.append(
            ...
            try:
                ...
                if not provider.optional:
                    raise CapabilityError("required_evidence_provider_guard_failed") from exc
                enabled = False
            ...
            if not enabled:
                if not provider.optional:
                    raise CapabilityError("required_evidence_provider_unavailable")
                statuses.append(
                    EvidenceProviderStatus(
                        provider_id=provider.provider_id,
                        state="unavailable",
    ...
    def search(
        ...
        *,
        ...
        available, initial_statuses = self.availability(context)
        selected = (routing_policy or EvidenceRoutingPolicy()).select(request, available)
        statuses = {item.provider_id: item for item in initial_statuses}
        ...
        for provider in selected:
            if len(refs) >= request.max_results:
                break
            remaining_bytes = request.max_total_bytes - used_bytes
            ...
            provider_request = replace(
                ...
                max_total_bytes=min(remaining_bytes, provider.max_total_bytes),
            )
            try:
                ...
                for ref in result.refs:
                    ...
                    if not ref.access_scope.allows(context):
                        continue
                    if ref.freshness in {EvidenceFreshness.MISSING, EvidenceFreshness.REVOKED}:
                        continue
                ...
                code = (
                    ...
                    else "evidence_provider_search_failed"
                )
    ...
    def fetch(
        ...
        *,
        ...
        if len(canonical_json_bytes(item.to_untrusted_projection())) > min(
            provider.max_total_bytes,
...
class EvidenceLedgerSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        refs_by_id = {item.evidence_id: item for item in self.refs}
        for item in self.items:
            ref = refs_by_id.get(item.ref.evidence_id)
            if ref is None:
                raise CapabilityError("evidence_ledger_orphan_item")
            if ref.provider_id != item.ref.provider_id or ref.locator.canonical_key != item.ref.locator.canonical_key:
    ...
    def from_json_value(cls, value: Mapping[str, Any]) -> "EvidenceLedgerSnapshot":
        ...
        items = tuple(
            EvidenceItem.from_json_value(item, refs_by_id=refs_by_id)
        ...
        return cls(
            ...
            items=items,
            captured_at=datetime.fromisoformat(
                str(value.get("captured_at") or utcnow().isoformat()).replace("Z", "+00:00")
            ),
        )
...
class EvidenceLedger:
    ...
    def register(
        ...
        *,
        ...
        proposed_refs = list(self._refs)
        proposed_items = dict(self._items)
        index = next(
            (index for index, item in enumerate(proposed_refs) if item.evidence_id == ref.evidence_id),
            None,
        )
        if index is None:
            if len(proposed_refs) >= LEDGER_MAX_REFS:
                raise CapabilityError("evidence_ledger_ref_limit_exceeded")
            proposed_refs.append(ref)
        else:
            existing = proposed_refs[index]
            if existing.provider_id != ref.provider_id or existing.locator.canonical_key != ref.locator.canonical_key:
                raise CapabilityError("evidence_ledger_identity_conflict")
            proposed_refs[index] = ref
            if ref.evidence_id in proposed_items:
                old = proposed_items[ref.evidence_id]
                proposed_items[ref.evidence_id] = EvidenceItem(
                    ref=ref,
                    excerpt=old.excerpt,
                    data=old.data,
                )
        if isinstance(value, EvidenceItem) and retain_excerpt:
            ...
            if len(excerpt.encode("utf-8")) > LEDGER_MAX_EXCERPT_BYTES:
                excerpt = excerpt.encode("utf-8")[:LEDGER_MAX_EXCERPT_BYTES].decode(
                    "utf-8", errors="ignore"
                )
            if ref.evidence_id not in proposed_items and len(proposed_items) >= LEDGER_MAX_EXCERPTS:
        ...
        snapshot = EvidenceLedgerSnapshot(
            ...
            refs=tuple(proposed_refs),
            items=tuple(
                proposed_items[item.evidence_id]
                for item in proposed_refs
                if item.evidence_id in proposed_items
            ),
            captured_at=captured_at,
        )
        self._revision = snapshot.revision
        self._refs = proposed_refs
        self._items = proposed_items
...
__all__ = [
    ...
    "LEDGER_MAX_EXCERPTS",
    "LEDGER_MAX_EXCERPT_BYTES",
    "LEDGER_MAX_REFS",
    "LEDGER_MAX_TOTAL_BYTES",
    "REDACTED_SECRET",
    "EvidenceAccessScope",
