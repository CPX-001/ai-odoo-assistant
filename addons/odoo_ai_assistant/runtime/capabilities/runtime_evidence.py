...
The provider deliberately uses the current user Environment and never accepts a
model-authored filesystem path, SQL fragment or method name. Mutable business
facts remain live ORM queries rather than inventory snapshots.
"""
...
def _service_inventory(env) -> Mapping[str, Any] | None:
    """Reuse the existing internal inventory service when it exposes a safe builder.

    Older snapshots used this module from an HTTP sidecar callback. P8 keeps the
    service internal. The adapter is deliberately narrow and falls back to live ORM
    when a compatible builder is not present.
    """
...
def collect_runtime_inventory(context: CapabilityContext) -> tuple[dict[str, JsonValue], str]:
    ...
    service_version = ""
    if service_payload:
        # Never project the service payload wholesale: older variants may contain
        # roots or operational details. Only a non-sensitive format/version marker
        # is reused here.
        service_version = str(
    ...
    # Roots, absolute paths, credentials and host process details are absent. A
    # future technical/host broker has its own policy and privilege boundary.
