...
def _module_payload(env, *, technical: bool) -> tuple[list[dict[str, JsonValue]], bool]:
    ...
    names = ["name", "state"]
    for field_name in ("shortdesc", "installed_version", "latest_version", "author", "license"):
...
def collect_runtime_inventory(context: CapabilityContext) -> tuple[dict[str, JsonValue], str]:
    ...
    # Roots, absolute paths, credentials and host process details are intentionally
    # absent.  A future technical/host broker has a separate policy boundary.
...
def _ref(
    ...
    *,
    ...
    return EvidenceRef(
        evidence_id=f"runtime:installation:{fingerprint[:24]}",
