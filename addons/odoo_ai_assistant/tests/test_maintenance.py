...
class FakeMaintenanceClient:
            ...
            "latest": [
                {
                    ...
                    "state": "succeeded",
                    "result_code": "readiness_ok",
                    "summary": "UNTRUSTED-BACKEND-TEXT",
                }
            ],
            "active_jobs": [],
        }
    ...
    def maintenance_action_self_test(self, payload):
        return self._result("action_self_test", "action_self_test_succeeded", payload)
...
class TestAssistantMaintenance(TransactionCase):
    def test_all_ui_operations_use_server_derived_actor_and_explicit_methods(self):
        ...
        actions = (
            "action_maintenance_readiness_test",
            "action_maintenance_source_rescan",
            "action_maintenance_source_test",
            "action_maintenance_logs_test",
            "action_maintenance_knowledge_reindex",
            "action_maintenance_reasoning_test",
            "action_maintenance_action_self_test",
            "action_maintenance_configuration_revalidate",
        )
    ...
    def test_status_renderer_ignores_backend_free_text_and_unknown_codes(self):
        ...
        values = diagnostics._maintenance_status_values(
            {
                "latest": [
                    {
                        "operation": "reasoning_test",
                        "state": "succeeded",
                        "result_code": "reasoning_auth_unavailable",
                        "summary": canary,
                    },
                    {
                        "operation": "reasoning_test",
                        "state": "succeeded",
                        "result_code": "backend_says_run_shell",
                        "summary": canary,
                    },
                ],
                "active_jobs": [],
    ...
    def test_non_admin_cannot_run_maintenance(self):
        user = self.env["res.users"].create(
            {
                "name": "M7 Maintenance Non Admin",
                "login": "m7-maintenance-non-admin",
        ...
        with self.assertRaises(AccessError):
            diagnostics.action_maintenance_action_self_test()

    def test_view_contains_only_explicit_maintenance_buttons(self):
        arch = self.env.ref(
            "odoo_ai_assistant.view_odoo_ai_assistant_diagnostics_maintenance_form"
        ).arch_db
        for action in (
            "action_maintenance_readiness_test",
            "action_maintenance_source_rescan",
        ...
        self.assertNotIn("/v1/admin/maintenance/run", arch)
