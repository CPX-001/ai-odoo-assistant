...
test("public activity and answer deltas stay on separate browser channels", async () => {
    const deltas = [];
    const activities = [];
    let statusCalls = 0;
    let liveCalls = 0;
...
test("transient polling reconnect reuses live cursor without duplicating answer", async () => {
    ...
    const result = await streamAssistantChatLive({
        ...
        fetchCall: async (path) => {
            if (path === "/odoo_ai/v1/turn") {
                return jsonResponse({ ok: true, turn_id: "turn-live-0001", state: "queued", last_sequence: 0, events: [] });
            }
            if (path === "/odoo_ai/v1/turn/live") {
                liveCalls += 1;
                if (liveCalls === 1) {
                ...
                return jsonResponse({ ok: true, turn_id: "turn-live-0001", items: [], last_sequence: 1, has_more: false });
            }
            statusCalls += 1;
            return jsonResponse({
                ok: true,
