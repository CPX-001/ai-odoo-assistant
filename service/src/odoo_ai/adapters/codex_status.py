...
class CachedCodexReasoningStatus:
    """Run a cheap account/handshake probe at most once per short TTL."""
    ...
    def from_env(
        ...
        settings = CodexRuntimeSettings.from_env(environ)
        source = os.environ if environ is None else environ
        raw_ttl = source.get(CODEX_READINESS_TTL_ENV)
        try:
            ttl = (
                DEFAULT_REASONING_PROBE_TTL_SECONDS
                if raw_ttl is None
                else float(raw_ttl)
            )
        except ValueError:
            raise ValueError("codex_readiness_ttl_invalid") from None
        return cls(settings=settings, ttl_seconds=ttl)
