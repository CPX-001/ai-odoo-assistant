...
Codex host-guidance vs untrusted-data partition
```
...
- addon installs/updates cleanly;
...
decision path so model-driven turns can request/select/fetch Evidence, persist the
bounded ledger across reconnect and surface provenance/citations. Then prepare the
six named P8 real gates. Do not add a second agent/runtime or retrieval registry.
