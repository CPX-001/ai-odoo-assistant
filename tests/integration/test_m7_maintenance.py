...
class FakeSourceDiagnostics:
    async def rescan_source(self) -> SourceScanDiagnostics:
        return SourceScanDiagnostics(state="DETECTED", scan_id=uuid4(), fingerprint="sha256:" + "a" * 64, metrics=SourceScanMetricsView(modules=2, files_seen=7, files_extracted=4, files_unchanged=3, bytes_hashed=1024, stale_files=0))
...
def _create_latest_profile(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        try:
            create_instance_profile(session, instance_id=f"maintenance-{uuid4()}", fingerprint=f"sha256:maintenance-{uuid4().hex}")
            session.flush()
        finally:
            session.close()
...
def test_async_job_duplicate_control_and_source_result_are_bounded(migrated_engine: Engine) -> None:
    service = RuntimeMaintenanceService(database_settings=DatabaseSettings.from_env(), diagnostics_factory=FakeSourceDiagnostics)
...
def test_knowledge_reindex_failure_is_retryable_and_rebuild_is_idempotent(migrated_engine: Engine, tmp_path: Path) -> None:
    _create_latest_profile(migrated_engine)
    root = tmp_path / "non-default knowledge maintenance root"
    environ = dict(os.environ)
    environ["ODOO_AI_KNOWLEDGE_SOURCES"] = json.dumps([{"provider_id": "pilot.docs", "root": str(root), "locale": "es-ES"}])
    service = RuntimeMaintenanceService(database_settings=DatabaseSettings.from_env(environ), environ=environ)
    ...
    with migrated_engine.connect() as connection:
        current = connection.scalar(text("SELECT count(*) FROM knowledge_document WHERE status = 'current'"))
    assert current is not None and current >= 1


def test_historical_action_maintenance_rows_do_not_break_current_status(migrated_engine: Engine) -> None:
    with migrated_engine.begin() as connection:
        connection.execute(text("INSERT INTO maintenance_audit_event (operation, state, actor_uid, actor_database, result_code, metrics) VALUES ('action_self_test', 'succeeded', 9, 'legacy', 'action_self_test_succeeded', '{}'::jsonb)"))
    service = RuntimeMaintenanceService(database_settings=DatabaseSettings.from_env())
    status = asyncio.run(service.status())
    assert all(item.operation != "action_self_test" for item in status.latest)
