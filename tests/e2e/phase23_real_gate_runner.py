...
def _odoo_selector(item):
    ...
    # Odoo syntax is [-][tag][/module][:class][.method]. A module-only selector
    # therefore needs the leading slash; tagged Phase 3 selectors do not.
...
def command(item):
    ...
    return cmd+["-u","odoo_ai_assistant","--test-enable","--test-tags",_odoo_selector(item),"--stop-after-init","--log-level=test"]
def main():
