...
def _payload(*, changes: tuple[ActionFieldChange, ...] | None = None, **updates: object):
...
def _create_payload(
    *, values: tuple[ActionFieldChange, ...] | None = None, **updates: object
) -> RecordCreateProposalPayload:
    ...
    return RecordCreateProposalPayload.model_validate(payload)
        ...
        ActionPolicy().validate_payload(
            _payload(
                changes=(
                    ActionFieldChange(
                        field="api_token",
                        value=ActionValue(kind=ActionValueKind.TEXT, value="not-a-secret"),
                    ),
                )
            )
        )
...
def test_create_canonicalization_is_stable_and_binds_all_authoritative_values() -> None:
    name = ActionFieldChange(
        field="name",
        value=ActionValue(kind=ActionValueKind.TEXT, value="New customer"),
    )
    reference = ActionFieldChange(
        field="parent_id",
        value=ActionValue(kind=ActionValueKind.MANY2ONE, value=42),
    )
    left = _create_payload(values=(name, reference))
    right = _create_payload(values=(reference, name))

    assert canonical_action_payload_bytes(left) == canonical_action_payload_bytes(right)
    assert action_payload_fingerprint(left) == action_payload_fingerprint(right)
    assert action_payload_fingerprint(left) != action_payload_fingerprint(
        _create_payload(values=(name,))
    )
    assert action_payload_fingerprint(left) != action_payload_fingerprint(
        _create_payload(target=ActionCreateTarget(model="sale.order"), values=(name, reference))
    )


def test_create_contract_rejects_record_id_extras_coercion_duplicates_and_caps() -> None:
    raw = _create_payload().model_dump(mode="json")
    with pytest.raises(ValidationError):
        RecordCreateProposalPayload.model_validate(
            {**raw, "target": {"model": "res.partner", "record_id": 42}}
        )
    with pytest.raises(ValidationError):
        RecordCreateProposalPayload.model_validate({**raw, "context": {"lang": "es_ES"}})
    with pytest.raises(ValidationError):
        RecordCreateProposalPayload.model_validate({**raw, "uid": "17"})
    with pytest.raises(ValidationError):
        RecordCreateProposalPayload.model_validate(
            {**raw, "values": [raw["values"][0], raw["values"][0]]}
        )
    with pytest.raises(ValidationError):
        RecordCreateProposalPayload.model_validate(
            {
...
def test_create_policy_blocks_sensitive_model_field_and_oversized_payload() -> None:
    ActionPolicy().validate_payload(_create_payload())

    with pytest.raises(ActionPolicyError, match="model_denied"):
        ActionPolicy().validate_payload(
            _create_payload(target=ActionCreateTarget(model="res.users"))
        )
    with pytest.raises(ActionPolicyError, match="field_denied"):
        ActionPolicy().validate_payload(
            _create_payload(
                values=(
                    ActionFieldChange(
                        field="password",
                        value=ActionValue(kind=ActionValueKind.TEXT, value="data"),
                    ),
                )
            )
        )
    with pytest.raises(ActionPolicyError, match="payload_too_large"):
        ActionPolicy(max_payload_bytes=256).validate_payload(_create_payload())


def _business_payload(**updates: object) -> BusinessActionProposalPayload:
    values: dict[str, object] = {
        "proposal_id": PROPOSAL_ID,
        "turn_id": TURN_ID,
        "instance_id": "odoo-production",
    ...
    values.update(updates)
    return BusinessActionProposalPayload.model_validate(values)


def test_curated_business_action_is_exact_parameter_free_and_fingerprinted() -> None:
    payload = _business_payload()
    ActionPolicy().validate_payload(payload)

    assert payload.action_id == "sale.order.confirm.v1"
    assert b"action_confirm" not in canonical_action_payload_bytes(payload)
    assert action_payload_fingerprint(payload) != action_payload_fingerprint(
        _business_payload(target=ActionTarget(model="sale.order", record_id=43))
    )
    ...
    for injected in (
        ...
        with pytest.raises(ValidationError):
            BusinessActionProposalPayload.model_validate({**raw, **injected})
    with pytest.raises(ValidationError):
        BusinessActionProposalPayload.model_validate({**raw, "action_id": "sale.order.unlink.v1"})
    with pytest.raises(ValidationError):
        BusinessActionProposalPayload.model_validate(
            {**raw, "target": {"model": "res.partner", "record_id": 42}}
        )
