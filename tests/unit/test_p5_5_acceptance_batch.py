...
SCRIPT = Path(__file__).resolve().parents[1] / "e2e" / "p5_5_acceptance_batch.py"
SPEC = importlib.util.spec_from_file_location("p5_5_acceptance_batch", SCRIPT)
...
def test_parse_odoo_summary_uses_real_odoo_result_shape():
    output = (
        ...
        "0 failed, 0 error(s) of 143 tests when loading database 'odoo_ai_p55'"
    )

    assert BATCH._parse_odoo_summary(output) == {"failed": 0, "errors": 0, "tests": 143}


def test_parse_browser_observation_requires_post_effect_gate_payload():
    output = "\n".join(
        [
            "browser startup",
            '{"gate":"OTHER","result":"PASS"}',
            '{"gate":"P5-REAL-POST-EFFECT","completed_effect_steps":1,'
    ...
    assert observation["completed_effect_steps"] == 1
