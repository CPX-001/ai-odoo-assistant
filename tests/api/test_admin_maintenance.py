...
class StubMaintenanceService(RuntimeMaintenanceService):
    ...
    async def action_self_test(self, actor: MaintenanceActor) -> MaintenanceResult:
        return self._result("action_self_test", "action_self_test_succeeded", actor)
    ...
    def _result(
        ...
        return MaintenanceResult.model_validate(
            {
                "operation": operation,
                "state": "succeeded",
                "result_code": code,
    ...
    def _enqueue(self, operation: str, actor: MaintenanceActor) -> MaintenanceJob:
        ...
        return MaintenanceJob.model_validate(
            {
                "job_id": JOB_ID,
                "operation": operation,
                "state": "queued",
...
def test_explicit_routes_require_auth_and_do_not_expose_generic_dispatch() -> None:
    ...
    for path in paths:
        response = asyncio.run(
            _request("POST", path, service=service, payload=_actor_payload())
        )
    ...
    unknown = asyncio.run(
        _request(
            ...
            service=service,
            payload={"operation": "shell", **_actor_payload()},
        )
    )
...
def test_status_and_job_are_bounded_machine_authenticated_views() -> None:
    service = StubMaintenanceService()
    status = asyncio.run(
        _request("GET", "/v1/admin/maintenance/status", service=service)
    )
    job = asyncio.run(
        _request("GET", f"/v1/admin/maintenance/jobs/{JOB_ID}", service=service)
    )
...
def test_invalid_oversized_and_duplicate_requests_fail_closed() -> None:
    ...
    invalid = asyncio.run(
        _request(
            ...
            "/v1/admin/maintenance/readiness/test",
            service=service,
            payload={**_actor_payload(), "operation": "reasoning_test"},
        )
    )
